...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT CAST(o.createdAt AS date), SUM(o.grandTotal) " +
           ...
           "GROUP BY CAST(o.createdAt AS date) ORDER BY 1 ASC")
    List<Object[]> revenueByDayForStore(@Param("storeId") Long storeId,
                                         @Param("from") LocalDateTime from,
                                         @Param("to") LocalDateTime to);
    
    @Query("SELECT CAST(COALESCE(o.createdAt, '1970-01-01') AS date), SUM(o.grandTotal) " +
           "FROM Order o WHERE o.store.id = :storeId " +
           "AND o.status NOT IN ('CANCELLED', 'RETURNED') " +
           "GROUP BY 1 ORDER BY 1 ASC")
    List<Object[]> revenueByDayForStoreLifetime(@Param("storeId") Long storeId);

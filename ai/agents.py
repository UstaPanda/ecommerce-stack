    ...
    try:
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a senior PostgreSQL expert for **ZorluKurt Trading**. Language: {detected_language}.\n"
                       ...
                       "PUBLIC DATA (no user filter, applies to ALL roles):\n"
                       ...
                       "- CORPORATE: \n"
                       "  * ALWAYS JOIN 'orders' with 'stores' on 'orders.store_id = stores.id'.\n"
                       "  * ALWAYS FILTER by 'stores.owner_id = {user_id}'.\n"
                       "- INDIVIDUAL: Filter by user_id = {user_id} on personal tables: 'orders', 'customer_profiles', 'carts'.\n\n"
                       "IMPORTANT RULES:\n"
                       "- REVENUE: Exclude 'CANCELLED' and 'RETURNED' orders.\n"
                       "- NEVER add user_id filter when querying 'products' or 'categories'.\n\n"
